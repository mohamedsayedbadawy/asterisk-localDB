...
@app.get("/search")
def search(
    q: Optional[str] = Query(None, description="Search query from AVA"),
    query: Optional[str] = Query(None, description="Search query alternative"),
    ...
    def clean(text):
        ...
        text = re.sub(r"\s+", " ", text).strip()
        return text
    combined = " ".join(clean(r["content"])[:400] for r in rows[:2])
...
def search_post(req: SearchRequest):
    q = req.q
    if not q or len(q.strip()) < 2:
        return {"results": [], "answer": "I don't have specific information about that."}
    conn = get_db()
    fts_query = " OR ".join(q.strip().split())
    try:
        rows = conn.execute("""
            SELECT f.page_num, f.content, d.filename, bm25(knowledge_fts) AS score
            FROM knowledge_fts f
            JOIN documents d ON d.id = f.doc_id
            WHERE knowledge_fts MATCH ?
    ...
    def clean(text):
        ...
        return text
    combined = " ".join(clean(r["content"])[:400] for r in rows[:2])
    return {"results": results, "answer": combined, "source_count": len(rows)}
